 ...
 * <p>
 * Add edges to a given graph from the current visitable to selected
 * visitables below the current one. By default, the selected
 * visitables are the immediate children.
 */
...
public class MkEdgesToKids extends VoidVisitor {
     ...
     * the current visitable is the edge source, and
     * <code>isKid</code> is used to select the edge targets.
     */
	...
	Visitor nodeAction
	    = new Sequence(isKid,new MkEdgeFromParent(parent,graph));
	Visitor v 
	    = new All(new TopDownUntil(nodeAction));
	v.visit(parent);
    }
}
